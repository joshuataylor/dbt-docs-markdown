...
The `dbt freshness` command evaluates whether sources and models with [freshness](../resource-configs/freshness.md) configured meet your `warn_after` and `error_after` thresholds, reporting warnings and errors accordingly.
...
`dbt freshness` evaluates sources or models that have `warn_after` or `error_after` set in their [`freshness` config](../resource-configs/freshness.md?version=2).
...
For the full schema, refer to: [`freshness.json`](../artifacts/freshness-json.md).
...
For the full schema, refer to [`sources.json`](../artifacts/sources-json.md).

The legacy [`dbt source freshness`](./source.md) command still works for backward compatibility and produces only `sources.json`.

## Related docs

* [Freshness config](../resource-configs/freshness.md)
* [Source data freshness](../../docs/build/sources.md#source-data-freshness)
