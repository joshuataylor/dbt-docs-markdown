...
By using the semantic manifest produced by dbt, MetricFlow will instantiate a data flow plan and generate SQL from Semantic Layer query requests. It's a valuable reference that you can use to understand the structure and details of your data models.
...
* Deserialization: dbt and MetricFlow use different libraries for handling data serialization.
* Efficiency and performance: MetricFlow and the dbt Semantic Layer need specific semantic details from the manifest. By trimming down the information printed into `semantic_manifest.json`, the process becomes more efficient and enables faster data handling between `dbt` and MetricFlow.
...
Starting in dbt v1.12, dbt also writes an `osi_document.json` file to your `target/` directory alongside `semantic_manifest.json` at parse time. This file represents your project's Semantic Layer in the [Apache Ossie](https://github.com/apache/ossie) format, a vendor-agnostic schema for describing semantic models and metrics.
