...
* **Produced by:** [`dbt freshness`](../commands/freshness.md)

This file contains freshness results for all sources and models with [freshness](../resource-configs/freshness.md) configured. dbt writes it to `target/freshness.json` after every `dbt freshness` invocation.
...
* `unique_id`: Unique node identifier, mapping results to `nodes` or `sources` in the [manifest](./manifest-json.md).
