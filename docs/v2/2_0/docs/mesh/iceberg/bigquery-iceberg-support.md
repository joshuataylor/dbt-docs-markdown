...
| ----------------------- | ------ | -------- | --------------------------------------------------------------------------------------- | ---------------------------- |
| `base_location_root`    | String | No       | If provided, the input will override the default dbt base\_location value of `_dbt`.    | Can be set in `catalogs.yml` |
...
| `storage_uri`           | String | No       | If provided, the input will override the dbt storage\_uri value.                        | Only configurable per-model  |
...
dbt also allows users to completely override the storage\_uri with the adapter property `storage_uri`. This overrides both the catalog integration path and any `base_location` overrides to supply the entire `storage_uri` path directly.
