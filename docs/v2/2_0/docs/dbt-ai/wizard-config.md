...
| `"fusion_cloud"`      | dbt platform handles deferral against your connected environment, so dbt Wizard doesn't manage any local state. Set automatically when you're connected to the platform.                                                                                                                                                                                  |
| `"cloud_cli"`         | The dbt platform CLI manages credentials and deferral through dbt platform, so dbt Wizard skips its production compile and deferral flag injection.                                                                                                                                                                                                       |
