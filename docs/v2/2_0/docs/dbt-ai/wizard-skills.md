...
To opt in, set `ai_provider` to one of the [supported providers list](./package-skills.md?version=2#set-the-ai_provider-flag). If you use Wizard, set `ai_provider` to `wizard` in your root project:
...
For the full setup, including how to ship skills in a package and disable ones you don't want, examples, and more — refer to [Installing agent skills from dbt packages](./package-skills.md).
