...
Sign in to connect the dbt VS Code extension to your dbt platform account. Signing in is what lets the extension reach your account for dbt platform capabilities, such as the [**Catalog** tab](./dbt-extension-features.md#explore-your-catalog) and [compare changes](./dbt/vs-compare-changes.md) with dbt platform deferral.
...
You can sign in from the command line using [`dbt login`](../reference/commands/login.md?version=2.0), available with v2 and later. Your login state can be shared across dbt features, including the dbt VS Code extension and, in supported versions, dbt State.
...
* Signing in links your editor to your dbt platform account so the extension can use dbt platform capabilities.
* You can authenticate with `dbt login` or sign in from inside the extension. If you don't have a dbt platform account, you can create a free account during authentication.
* If a valid [`dbt_cloud.yml`](../reference/dbt_cloud.yml.md) file exists on your machine, the extension can use it automatically.

If you've signed in before, you won't need to authenticate again unless your session expired.
...
There are a couple of different ways to sign in to a dbt platform account. Choose the best path for you:
...
| Are new to dbt and don't have a dbt platform account                  | [First-time sign-in](./sign-in-dbt-extension.md?registration-path=first-time#choose-a-sign-in-path) from the dbt VS Code extension.    |
...
The extension can now reach your dbt platform account. For details on what `dbt login` does across tools, refer to the [`dbt login` reference](../reference/commands/login.md?version=2.0).

### First-time registration

### First-time registration

Create a dbt platform account and connect it to the extension. Use this path if you've never signed in before. The extension and the CLI open the same browser flow, so you can start from either entry point:
