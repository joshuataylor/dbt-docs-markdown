...
To continue using all the features, register or log in from the command line using [`dbt login`](https://docs.getdbt.com/reference/commands/login.md), available with v2 and later. Your login state can be shared across dbt features, including the dbt VS Code extension and, in supported versions, dbt State.
