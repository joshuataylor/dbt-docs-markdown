...
Note: dbt Core supports using the `DBT_ENGINE_PROFILES_DIR` environment variable or a `profiles.yml` file in the current working directory. These options aren't currently supported in Fusion.
...
You can also manually create the file and add it to the proper location. To configure an adapter manually, copy and paste the fields from the adapter setup instructions for [dbt Core](../../local/connect-data-platform/about-dbt-connections.md) or [Fusion](../../local/profiles.yml.md) along with the appropriate values for each.
...
dbt supports multiple targets within one profile to encourage the use of separate development and production environments as discussed in [dbt environments](../../local/dbt-core-environments.md).
...
* [Connection profiles](../../local/connection-profiles.md)
