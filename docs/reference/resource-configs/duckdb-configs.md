...
dbt users don't have to create their own profiles.yml file. dbt-duckdb [profiles](https://docs.getdbt.com/docs/core/connect-data-platform/duckdb-setup.md#connecting-to-duckdb-with-dbt-duckdb) should be set up as follows:
...
As of `dbt-duckdb 1.5.2`, you can connect to a DuckDB instance running on [MotherDuck](https://motherduck.com) by setting your path to use an `md:` connection string, just as you would with the DuckDB CLI or the Python API. MotherDuck databases generally work the same way as local DuckDB databases from the perspective of dbt, but there are a few differences to be aware of:
...
dbt added support for [Python models](https://docs.getdbt.com/docs/build/python-models.md) in version `1.3.0`. For most data platforms, dbt will package up the Python code defined in a `.py` file and ship it off to be executed in whatever Python environment that data platform supports (for example, Snowpark for Snowflake or Dataproc for BigQuery).
...
| options         | None                      | Any other options to pass to DuckDB's COPY operation (for example partition\_by, codec, etc). |
...
`dbt-duckdb` supports the `delete+insert` and `append` [strategies](https://docs.getdbt.com/docs/build/incremental-strategy.md#built-in-strategies) for incremental table models, but there's no support for incremental materialization strategies for external models.
