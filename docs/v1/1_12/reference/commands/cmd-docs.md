...
With dbt v2, generate docs metadata by running `dbt build --write-catalog` (or `dbtf build --write-catalog`). Metadata is not generated by default with `dbt build` alone. To view it in Catalog, run a job in dbt platform so the metadata is uploaded, then open [Catalog](../../docs/explore/build-and-view-your-docs.md).
...
Use `dbt docs serve` if you're developing locally with the [dbt platform CLI](../../docs/platform/dbt-cli-installation.md) or [dbt v1](../../docs/local/install-dbt.md). The [Studio IDE](../../docs/platform/studio-ide/develop-in-studio.md) doesn't support this command.
...
The `--host` flag is only available in the [dbt v1](../../docs/local/install-dbt.md). It's not supported in the [dbt platform CLI](../../docs/platform/dbt-cli-installation.md).
