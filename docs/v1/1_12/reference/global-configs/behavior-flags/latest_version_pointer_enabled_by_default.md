...
| latest\_version\_pointer\_enabled\_by\_default | dbt **v1 Latest** | dbt v1 | dbt v2         |
| ---------------------------------------------- | ----------------- | ------ | -------------- |
| Introduced                                     | 2026.5            | 1.12.0 | Early preview  |
| Matured (default → `true`)                     | —                 | —      | Already `true` |
| Removed                                        | —                 | —      | —              |

The `latest_version_pointer_enabled_by_default` flag is set to `false` by default in dbt v1. In dbt v2, this flag defaults to `true`, which enables the latest version pointer for all versioned models automatically.
