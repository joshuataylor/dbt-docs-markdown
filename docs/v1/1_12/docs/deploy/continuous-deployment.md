...
To start the same merge or deployment job from your own tooling, use the [Administrative API](../dbt-apis/admin-api.md) and [Trigger Job Run](https://docs.getdbt.com/dbt-cloud/api-v2#/operations/Trigger%20Job%20Run). Configure and validate the job in dbt first, note the account and job identifiers from the deployment URL or job settings, authenticate with [service tokens](../dbt-apis/service-tokens.md) or [personal access tokens](../dbt-apis/user-tokens.md), assemble the HTTPS request headers and JSON body from [Trigger Job Run](https://docs.getdbt.com/dbt-cloud/api-v2#/operations/Trigger%20Job%20Run), then monitor the resulting run alongside any webhook-triggered executions in job history.
