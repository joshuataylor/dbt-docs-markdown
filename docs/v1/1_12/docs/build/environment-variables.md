...
* `DBT_CLOUD_RUN_REASON` — The specific trigger for this run (for example, `Scheduled`, `Kicked off by <email>`, or a custom value set through the API)
...
Env vars work well with username/password and keypair, including scheduled jobs, because dbt v1 consumes the Jinja inserted into the autogenerated [`profiles.yml`](../local/profiles.yml.md) and resolves it to do an `env_var` lookup.
