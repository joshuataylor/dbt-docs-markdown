...
3. Add a `dbt` entry under the top-level `servers` key. (VS Code uses `servers`, not `mcpServers`.) Pick the tab that matches your auth method:
   ...
   * **`x-dbt-user-id`** — required for `execute_sql` with token-based auth. Refer to [Where can I find my user ID?](../../faqs/Accounts/find-user-id.md) for details.
   ...
   If you use token-based auth, `execute_sql` requires a personal access token (PAT). Service tokens won't work. To run SQL without creating a PAT, connect with [OAuth (remote MCP)](./setup-remote-mcp.md#oauth-remote-mcp) instead. Refer to [Set up remote MCP](./setup-remote-mcp.md#token-based-authentication) for the full list of headers.
