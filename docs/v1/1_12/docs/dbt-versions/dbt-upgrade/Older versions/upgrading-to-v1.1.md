...
For users of [state-based functionality](../../../../reference/node-selection/syntax.md#about-node-selection), such as the `state:modified` selector, recall that:
...
[**Generic tests**](../../../../reference/resource-properties/data-tests.md) can define custom names. This is useful to "prettify" the synthetic name that dbt applies automatically. It's needed to disambiguate the case when the same generic test is defined multiple times with different configurations.
...
[**dbt-Jinja functions**](../../../../reference/dbt-jinja-functions-context-variables.md) have a new landing page, and two new members:

* [`print`](../../../../reference/dbt-jinja-functions/print.md) exposes the Python `print()` function. It can be used as an alternative to `log()`, and together with the `QUIET` config, for advanced macro-driven workflows.
* [`selected_resources`](../../../../reference/dbt-jinja-functions/selected_resources.md) exposes, at runtime, the list of DAG nodes selected by the current task.

[**Global configs**](../../../../reference/global-configs/about-global-configs.md) include some new additions:
