...
* [ ] Whether starting a brand new project or importing an existing dbt v1 project, you'll want to make sure you have the [proper structure configured](./build/projects.md).
  * [ ] If you are migrating from dbt v1, there are some important things you'll need to know, so check out our [migration guide](../guides/dbt-migration-2.md?step=1).
...
* [ ] dbt models are primarily written as [SELECT statements](./build/sql-models.md), so an early step for measuring success is having a developer run a simple select statement in the IDE and validating the results.
  * [ ] You can also verify the connection by running basic SQL queries using [dbt Insights](./explore/access-dbt-insights.md).
* [ ] Create a single model and ensure that you can [run it](../reference/dbt-commands.md) successfully.
  * [ ] For an easy to use drag-and-drop interface, try creating it with [dbt Canvas](./platform/canvas.md).
* [ ] Create a service account with proper access for your [production jobs](./deploy/jobs.md).
...
* [ ] Configure a [Git repository](./platform/git/configure-git.md) for your account. dbt supports integrations with:
  ...
  * Other providers using [Git clone](./platform/git/import-a-project-by-git-url.md)
  * If you aren't ready to integrate with an existing Git solution, dbt can provide you with a [managed Git repository](./platform/git/managed-repository.md).

* [ ] Ensure developers can [checkout](./platform/git/version-control-basics.md#git-overview) a new branch in your repo.

* [ ] Ensure developers in the IDE can [commit changes](./platform/studio-ide/ide-user-interface.md#basic-layout).

## Environments and jobs

[Environments](./environments-in-dbt.md) separate your development data from your production data. dbt supports two environment types: Development and Deployment. There are three types of deployment environments:
...
[Jobs](./deploy/jobs.md) dictate which commands are run in your environments and can be triggered manually, on a schedule, by other jobs, by APIs, or when pull requests are committed or merged.

Once you connect your data warehouse and complete the Git integration, you can configure environments and jobs:

* [ ] Start by creating a new [Development environment](./dbt-platform-environments.md#create-a-development-environment) for your project.
* [ ] Create a [Production Deployment environment](./deploy/deploy-environments.md).
  * [ ] (Optional) Create an additional Staging or General environment.
* [ ] [Create and schedule](./deploy/deploy-jobs.md#create-and-schedule-jobs) a deployment job.
...
* [ ] Manually [invite users](./platform/manage-access/invite-users.md) to the dbt platform, and they can authenticate using [MFA (SMS or authenticator app)](./platform/manage-access/mfa.md).
* [ ] Configure [single sign-on or OAuth](./platform/manage-access/sso-overview.md) for advanced access control. [Enterprise](https://www.getdbt.com/pricing "Go to https://www.getdbt.com/pricing")[Enterprise +](https://www.getdbt.com/pricing "Go to https://www.getdbt.com/pricing") accounts only.
  * [ ] Create [SSO mappings](./platform/manage-access/about-user-access.md#sso-mappings-) for groups
* Configure [System for Cross-Domain Identity Management (SCIM)](./platform/manage-access/scim.md) if available for your IdP.
* [ ] Ensure invited users are able to connect to the data warehouse from their personal profile.
* [ ] [Create groups](./platform/manage-access/about-user-access.md#create-new-groups-) with granular permission sets assigned.
