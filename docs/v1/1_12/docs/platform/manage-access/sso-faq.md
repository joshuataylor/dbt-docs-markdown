...
 Where do I find my Auth0 URI for configuring SSO?

Your Auth0 URI is displayed on the **Single Sign-On** settings page in dbt platform under **Account settings → Single Sign-On**.
...
Before enabling SSO enforcement, ensure that at least one account admin can successfully sign in through SSO. If SSO is misconfigured and enforcement is enabled, admins may be locked out of the account. Contact <support@getdbt.com> if this occurs.

 What Entra ID permissions does the dbt SSO integration require?
...
 Can I use different IdPs for SSO and SCIM?

SSO and SCIM are configured independently in dbt platform, but should use the same IdP. Using different providers — for example, Okta for SSO and Microsoft Entra ID for SCIM — is not supported and can cause unintended behavior.
...
When a user signs in to a dbt platform account through SSO for the first time, a verification email is sent to confirm their identity. If the email is not received, it's usually caused by one of the following:
...
* **The email is being suppressed** in our email delivery provider. Contact <support@getdbt.com> to check delivery logs for the affected address.

 Users are prompted for MFA twice when signing in
...
 "Access Denied" redirect after a successful SAML authentication

The SAML assertion succeeds on the IdP side, but dbt platform is rejecting the response. This is almost always caused by a mismatch between the values configured in the IdP and what dbt platform expects.

To resolve, verify all the SSO configuration values in dbt platform match what was configured in your IdP

 New users see an "Admin consent required" prompt after SSO is set up
