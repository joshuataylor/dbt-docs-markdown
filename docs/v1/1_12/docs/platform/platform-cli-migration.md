...
* **How do I tell which one I have installed?** `dbt --version`. The dbt platform CLI prints a version like `0.40.20`; the corresponding version on PyPI is `1.0.0.40.20`. dbt v2's version string needs reconfirming post-update. At the time this article was published, dbt v2's binary still internally identifies as `dbt-fusion X.Y.Z`, which may change.
