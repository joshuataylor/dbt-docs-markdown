...
We recommend using the default behavior, but if you need to customize the resulting `base_location`, dbt allows users to configure the base\_location with the model configuration fields `base_location_root` and `base_location_subpath`.
