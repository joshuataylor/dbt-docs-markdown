# Onboarding to Lake Compute [Private beta](https://docs.getdbt.com/docs/dbt-versions/product-lifecycles "Go to https://docs.getdbt.com/docs/dbt-versions/product-lifecycles")
...
### 2. Create an MDLS destination (if you don't already have one)
...
![Enable Lake Compute](/img/docs/lake-compute/enable-lake-compute.png?v=2 "Enable Lake Compute")Enable Lake Compute
...
![Generate a Lake Compute token](/img/docs/lake-compute/generate-token.png?v=2 "Generate a Lake Compute token")Generate a Lake Compute token
...
This section applies to Option 1 (PAT) only
...
As a temporary workaround during private beta, you can set Lake Compute configurations using [Extended Attributes](../dbt-platform-environments.md#extended-attributes), by adding a key named `lakecompute`:
...
All your models run on Lake Compute by default and write to Iceberg tables in MDLS. Also check for existing `materialized: view` configs — Lake Compute doesn't support views (see [Private beta limitations](../lake-compute.md#private-beta-limitations)), so switch any views to `table` or `ephemeral` before converting a project over.
