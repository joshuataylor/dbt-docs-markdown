...
For OAuth *without* a self-hosted installation, use the [remote MCP server](../docs/dbt-ai/mcp-quickstart-remote.md). If your client does not support OAuth or you need token-based access, use [token-based authentication](../docs/dbt-ai/setup-remote-mcp.md#token-based-authentication).
...
1. Generate a [Personal Access Token (PAT)](../docs/dbt-apis/user-tokens.md) in **Account settings** → **API tokens** → **Personal tokens**.
