...
The Semantic Layer currently supports Snowflake, BigQuery, Databricks, Redshift, Postgres, and Trino (including Starburst Galaxy). It does not support Microsoft Fabric. For the full list, refer to [What data platforms are supported](../docs/use-dbt-semantic-layer/sl-faqs.md#what-data-platforms-are-supported-by-the-dbt-semantic-layer).
...
You can also complete the [Quickstart for dbt and Snowflake](./snowflake.md) first if you prefer a longer Snowflake-only walkthrough.
...
* [Step 3: Create BigQuery datasets](./bigquery.md?step=3)
...
* [Step 5: Set up the integration from Partner Connect](./databricks.md?step=5)
...
* [Step 4: Connect dbt to Redshift](./redshift.md?step=4)
...
* [Step 3: Connect Starburst Galaxy to the Amazon S3 bucket](./starburst-galaxy.md?step=3)
...
If you used Snowflake or Databricks Partner Connect, you can skip to [initializing your dbt project](#initialize-your-dbt-project-and-start-developing). Partner Connect provides a [managed repository](../docs/platform/git/managed-repository.md). Otherwise, you will need to create your repository connection.
...
[Staging models](../best-practices/how-we-structure/2-staging.md) are the first transformation step in dbt. They clean and prepare your raw data, making it ready for more complex transformations and analyses.
...
In this section, you'll learn about [semantic model](./sl-qs.md?step=5#about-semantic-models), [their components](./sl-qs.md?step=5#semantic-model-components), and [how to configure a time spine](./sl-qs.md?step=5#configure-a-time-spine).

### About semantic models

(Applies to dbt v1.12 and later)

[Semantic models](../docs/build/semantic-models.md) contain many object types (such as entities, simple metrics, and dimensions) that allow MetricFlow to construct the queries for metric definitions.
...
The following sections explain [dimensions](../docs/build/dimensions.md), [entities](../docs/build/entities.md), and [simple metrics](../docs/build/metrics-overview.md#simple-metrics) in more detail, showing how they each play a role in semantic models.
...
[Simple metrics](../docs/build/simple.md) perform an aggregation (like `sum`, `count`, or `average`, and so on) on a single field in your model. They replace the concept of "measures" in previous versions. To define more advanced metrics, refer to [Define metrics and add a second semantic model](./sl-qs.md?step=6).
...
Once you've created your semantic models, it's time to start referencing those (Applies to dbt v1.12 and later) simple metrics you made to create some metrics:
...
1. Once you’ve [defined your semantic models and metrics](./sl-qs.md?step=6), commit and merge your metric changes in your dbt project.
...
6. Set the job to run a `dbt parse` job to parse your projects and generate a [`semantic_manifest.json` artifact](../reference/artifacts/sl-manifest.md) file(Applies to dbt v1.12 and later) and an [`osi_document.json` artifact](../reference/artifacts/sl-manifest.md#apache-ossie-document). Although running `dbt build` isn't required, you can choose to do so if needed.
...
![Select 'Data browser' and then 'Add a data connection'. Choose the warehouse type that matches your dbt project.](/img/docs/dbt-platform/semantic-layer/hex_new_data_connection.png?v=2 "Select 'Data browser' and then 'Add a data connection'. Choose the warehouse type that matches your dbt project.")Select 'Data browser' and then 'Add a data connection'. Choose the warehouse type that matches your dbt project.
...
* Set up your warehouse and dbt, including loading sample data and connecting your project.
