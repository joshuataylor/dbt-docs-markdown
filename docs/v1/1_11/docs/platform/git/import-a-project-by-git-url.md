...
* **The URL is invalid**: Confirm the repo path is correct, the repo exists, and you are using an SSH clone URL (`git@...` or `ssh://...`). Do not use `https://...` or other HTTP URLs.
* **Your git provider is unavailable**: Check your provider's status page (for example, GitHub, GitLab, Bitbucket, Azure DevOps) and retry after service is restored.
* **SSH access isn't configured correctly**: Make sure the deploy key generated by dbt is added to your repository with the required access (including write access where applicable). If your organization restricts SSH keys, coordinate with your admins to allow deploy keys for the repository.
...
[![Navigate to the 'SSH public keys' settings page](/img/docs/dbt-platform/platform-configuring-dbt-platform/52bfdaa-Screen_Shot_2020-03-09_at_4.13.20_PM.png?v=2 "Navigate to the 'SSH public keys' settings page")](#)Navigate to the 'SSH public keys' settings page

3. Next, click the **+ New Key** button to create a new SSH key for the repository.

[![Click the '+ New Key' button to create a new SSH key for the repository.](/img/docs/dbt-platform/platform-configuring-dbt-platform/6d8e980-Screen_Shot_2020-03-09_at_4.13.27_PM.png?v=2 "Click the '+ New Key' button to create a new SSH key for the repository.")](#)Click the '+ New Key' button to create a new SSH key for the repository.
