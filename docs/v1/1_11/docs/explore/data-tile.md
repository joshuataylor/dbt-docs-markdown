...
2. Copy the URL or iFrame snippet available in Catalog's **Data health** section, under the **Embed data health into your dashboard** toggle.

3. Insert a **Web Page** object.

4. Paste the copied URL and click **Ok**.

   The URL you copy from Catalog looks similar to the following but may include additional Tableau-specific parameters (such as `biResourceId` and `autoBiProvider=tableau`) that are required to resolve the exposure:
