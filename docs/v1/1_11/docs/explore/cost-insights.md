...
Snowflake attribution limitation
...
On June 1, 2026, dbt Labs and Fivetran announced **[dbt State](../deploy/dbt-state-about.md)** as a new and improved version of state-aware orchestration. A key feature is [`lag_tolerance`](../../reference/resource-configs/lag-tolerance.md), which controls how much time must pass since the last upstream data change before a node is eligible for a rebuild.
