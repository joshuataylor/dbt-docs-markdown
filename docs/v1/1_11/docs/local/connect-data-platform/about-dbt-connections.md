...
You can measure source freshness using warehouse metadata tables on supported adapters. This calculates source freshness without using the [`loaded_at_field`](../../../reference/resource-configs/freshness.md) and without querying the table directly. This approach is faster and more flexible (though it might sometimes be inaccurate, depending on how the warehouse tracks altered tables). You can override this with the `loaded_at_field` in the [source config](../../../reference/source-configs.md). If the adapter doesn't support this, you can still use the `loaded_at_field`.
