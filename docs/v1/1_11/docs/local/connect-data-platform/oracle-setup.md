...
Starting with `dbt-oracle==1.8.0` database key in `profiles.yml` is **still optional for all but one** of the dbt-oracle workflows. if `database` is missing in `profiles.yml` the generated catalog used for project documentation will be empty.
