...
* Write your first five charts — a line, bar, area, pie, and table — from your dbt models.
...
* You have [dbt v1.12+ installed](../docs/local/install-dbt.md?version=2). This guide assumes dbt v2.
...
* [Build your dbt models](../docs/build/models.md)
* [How we structure our dbt projects](../best-practices/how-we-structure/1-guide-overview.md)
...
Record the database and schema that dbt built into — your chart SQL references them. To find them, run `dbt list --output json` and read each model's `relation_name`.
...
Now add five charts to `charts/analytics.yml`, one for each of the most common chart types. Each chart references a SQL query — defined in a `queries:` block — and maps that query's output columns to the chart's channels (`x`, `y`, `theta`, `color`).

Each snippet below shows the query and the chart to add. Charts don't render until you add the layout in the next step, and the complete file appears in [Lay out your board](./dbt-charts.md?step=5).
...
* **`dbt_charts.yml` validation error: extra inputs are not permitted** — `dbt_charts.yml` accepts a fixed set of top-level keys, such as `sources` and `server`. Remove anything else, including a `name` key.
* **A chart shows no data** — Confirm your `profiles.yml` connection works (`dbt debug`) and that the profile's role can `SELECT` from the schemas your queries read.
* **Some charts show data but others are all zeros** — Check whether the empty charts read a model that your dev target filters. A common pattern limits development data to a recent time window, such as a `limit_in_dev` macro on orders. On a static demo dataset, that window can exclude every row and zero out the models downstream of it, while models without the filter still populate. Anchor the window to the data's own `max()` timestamp, widen it, or build against a target that skips the filter.
* **Table or view not found** — Confirm the database and schema prefix on each `FROM` matches where dbt actually builds the model. Run `dbt list --output json` and read `relation_name` to get the exact location.
* **Column not found** — A chart's channels (`x`, `y`, `theta`, `color`) must match the column names your query's `SELECT` returns. Alias columns in SQL so the names line up.
* **A chart renders blank** — Confirm the chart's `type` supports the channels you set. Sector charts (pie and donut) use `theta` and `color`, not `x` and `y`.
* **`dct validate` warns about re-aggregation** — The cumulative revenue query sums an already-aggregated column on purpose, so you can ignore this warning. `dct validate` still exits successfully.
...
* Add more files under `charts/` — each one becomes its own page, so you can break a large dashboard into focused boards.
