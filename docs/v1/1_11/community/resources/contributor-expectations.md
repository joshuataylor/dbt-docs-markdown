...
| `engine:`     | `engine:v1`, `engine:v2`                                                                                                          | Which dbt engine the issue concerns: `engine:v1` is the Python engine (dbt Core v1.x); `engine:v2` is the v2 Rust rewrite that powers dbt Core 2.0 and the dbt Fusion engine. |
...
* [(dbt-core#9347) Fix configuration of turning test warnings into failures](https://github.com/dbt-labs/dbt-oss/pull/9347)
* [(dbt-core#9863) Better error message when trying to select a disabled model](https://github.com/dbt-labs/dbt-oss/pull/9863)
