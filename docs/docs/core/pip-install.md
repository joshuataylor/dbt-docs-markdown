...
Users who want to run dbt locally, for example in [dbt Core](https://docs.getdbt.com/docs/core/installation-overview.md) or the [dbt CLI](https://docs.getdbt.com/docs/cloud/cloud-cli-installation.md#install-a-virtual-environment) may want to install a Python virtual environment.
...
If you're using the dbt CLI, you can [install dbt CLI in pip](https://docs.getdbt.com/docs/cloud/cloud-cli-installation.md#install-dbt-cloud-cli-in-pip) after creating your virtual environment.
...
You can upgrade or downgrade versions of dbt Core by using the `--upgrade` option on the command line (CLI). For more information, see [Best practices for upgrading in Core versions](https://docs.getdbt.com/docs/dbt-versions/core.md#best-practices-for-upgrading).
...
In the fall of 2023, the `dbt` package on PyPI became a supported method to install the [dbt CLI](https://docs.getdbt.com/docs/cloud/cloud-cli-installation.md?install=pip#install-dbt-cloud-cli-in-pip).
