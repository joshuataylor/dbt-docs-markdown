...
**dbt-spark** added support for a [`session` connection method](/docs/local/connect-data-platform/spark-setup#session), for use with a pySpark session, to support rapid iteration when developing advanced or experimental functionality. This connection method is not recommended for new users, and it is not supported in <Constant name="dbt" />.
