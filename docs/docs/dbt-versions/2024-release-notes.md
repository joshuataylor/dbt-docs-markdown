...
- **New:** The [Amazon Athena warehouse connection](/docs/platform/connect-data-platform/connect-amazon-athena) is available as a public preview for <Constant name="dbt" /> accounts that have upgraded to [the **Latest** release track](/docs/dbt-versions/cloud-release-tracks).
...
- **New**: The ability to lint your SQL files from the <Constant name="dbt" /> CLI is now available. To learn more, refer to [Lint SQL files](/docs/platform/configure-cloud-cli#lint-sql-files).
...
- **New:** [<Constant name="dbt" /> CLI](/docs/platform/cloud-cli-installation) is now Generally Available (GA) to all users. The <Constant name="dbt" /> CLI is a command-line interface that allows you to interact with <Constant name="dbt" />, use automatic deferral, leverage <Constant name="mesh" />, and more!
...
- **Update**: The [<Constant name="dbt" /> CLI](/docs/platform/cloud-cli-installation) is now available for Azure single tenant and is accessible in all [deployment regions](/docs/platform/about-platform/access-regions-ip-addresses) for both multi-tenant and single-tenant accounts.
