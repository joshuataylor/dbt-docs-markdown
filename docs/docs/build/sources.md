...
### Fusion and state-aware orchestration[​](#fusion-and-state-aware-orchestration "Direct link to Fusion and state-aware orchestration")
...
* Define custom freshness logic using [advanced configurations](https://docs.getdbt.com/docs/deploy/state-aware-setup.md#advanced-configurations) `loaded_at_field` or `loaded_at_query` (for example, for streaming data or partial loads).
