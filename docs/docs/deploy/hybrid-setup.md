...
5. After creating a project, create a corresponding [production environment](https://docs.getdbt.com/docs/deploy/deploy-environments.md#create-a-deployment-environment) and click **Save**. Note that you can leave the **Connection** field blank.
...
2. If you don't have the latest version (1.10 or later), [upgrade](https://docs.getdbt.com/docs/core/pip-install.md#change-dbt-core-versions) your dbt Core project by running `python -m pip install --upgrade dbt-core`.
