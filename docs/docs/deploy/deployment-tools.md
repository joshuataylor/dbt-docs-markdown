...
Alongside [<Constant name="dbt" />](/docs/deploy/jobs), discover other ways to schedule and run your dbt jobs with the help of tools such as the ones described on this page.

Build and install these tools to automate your data workflows, trigger dbt jobs (including those hosted on <Constant name="dbt" />), and enjoy a hassle-free experience, saving time and increasing efficiency.
...
Installing the [<Constant name="dbt" /> Provider](https://airflow.apache.org/docs/apache-airflow-providers-dbt-cloud/stable/index.html) to orchestrate <Constant name="dbt" /> jobs. This package contains multiple Hooks, Operators, and Sensors to complete various actions within <Constant name="dbt" />.
...
3. Securely fetch the <Constant name="dbt" /> service token from a key vault in ADF, using a web call as the first step in the pipeline.
4. Set the parameters in the pipeline, including the <Constant name="dbt" /> account ID and  job ID, as well as the name of the key vault and secret that contains the service token. 
    * You can find the <Constant name="dbt" /> job and account id in the URL, for example, if your URL is `https://YOUR_ACCESS_URL/deploy/88888/projects/678910/jobs/123456`, the account ID is 88888 and the job ID is 123456
5. Trigger the pipeline in ADF to start the <Constant name="dbt" /> job and monitor the status of the <Constant name="dbt" /> job in ADF.
6. In <Constant name="dbt" />, you can check the status of the job and how it was triggered in <Constant name="dbt" />.
...
If your organization uses [Dagster](https://dagster.io/), you can use the [dagster_dbt](https://docs.dagster.io/_apidocs/libraries/dagster-dbt) library to integrate dbt commands into your pipelines. This library supports the execution of dbt through <Constant name="dbt" /> or <Constant name="core" />. Running dbt from Dagster automatically aggregates metadata about your dbt runs. Refer to the [example pipeline](https://dagster.io/blog/dagster-dbt) for details.
...
If your organization uses [Kestra](http://kestra.io/), you can leverage the [dbt plugin](https://kestra.io/plugins/plugin-dbt) to orchestrate <Constant name="dbt" /> and <Constant name="core" /> jobs. Kestra's user interface (UI) has built-in [Blueprints](https://kestra.io/docs/user-interface-guide/blueprints), providing ready-to-use workflows. Navigate to the Blueprints page in the left navigation menu and [select the dbt tag](https://demo.kestra.io/ui/blueprints/community?selectedTag=36) to find several examples of scheduling <Constant name="core" /> commands and <Constant name="dbt" /> jobs as part of your data pipelines. After each scheduled or ad-hoc workflow execution, the Outputs tab in the Kestra UI allows you to download and preview all dbt build artifacts. The Gantt and Topology view additionally render the metadata to visualize dependencies and runtimes of your dbt models and tests. The <Constant name="dbt" /> task provides convenient links to easily navigate between Kestra and <Constant name="dbt" /> UI.

## Orchestra

If your organization uses [Orchestra](https://getorchestra.io), you can trigger dbt jobs using the <Constant name="dbt" /> API. Create an API token from your <Constant name="dbt" /> account and use this to authenticate Orchestra in the [Orchestra Portal](https://app.getorchestra.io). For details, refer to the [Orchestra docs on <Constant name="dbt" />](https://orchestra-1.gitbook.io/orchestra-portal/integrations/transformation/dbt-cloud).
...
The following is an example of the run details in <Constant name="dbt" /> for a job triggered by Orchestra:
...
If your organization uses [Prefect](https://www.prefect.io/), the way you will run your jobs depends on the dbt version you're on, and whether you're orchestrating <Constant name="dbt" /> or <Constant name="core" /> jobs. Refer to the following variety of options:
...
- Trigger <Constant name="dbt" /> jobs with the [DbtCloudRunJob](https://docs.prefect.io/api/latest/tasks/dbt.html#dbtcloudrunjob) task. 
