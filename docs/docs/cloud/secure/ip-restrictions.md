...
Organizations can configure IP restrictions using the following dbt Enterprise tiers:
...
Once enabled, when someone attempts to access dbt from a restricted IP, they encounter one of the following messages depending on whether they use email and password or SSO login:
