...
- [Orchestrate exposures](/docs/platform-integrations/orchestrate-exposures) to refresh the underlying data sources during scheduled dbt jobs, improving timeliness and reducing costs. Orchestrating exposures is essentially a way to ensure that your BI tools are updated regularly by using the [<Constant name="dbt" /> job scheduler](/docs/deploy/deployments).
  - For more info on the differences between visualizing and orchestrating exposures, see [Visualize and orchestrate downstream exposures](/docs/platform-integrations/downstream-exposures).

To configure downstream exposures automatically from dashboards in Tableau, prerequisites, and more &mdash; refer to [Configure downstream exposures](/docs/platform-integrations/downstream-exposures-tableau).
