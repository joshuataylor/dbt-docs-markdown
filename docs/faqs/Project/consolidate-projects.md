...
Consolidating your dbt projects can be an enormous task, and there is no universal solution. But, there are some common approaches to project consolidation in <Constant name="dbt" /> that you can follow, depending on the scope of the work that needs to be done.
...
If you only have a couple of models or macros that you want to consolidate, copy the raw file contents from your git provider in `Smaller Subset Project`. Then, in the <Constant name="studio_ide" />, paste the contents into a new file in your `Main Project`.
