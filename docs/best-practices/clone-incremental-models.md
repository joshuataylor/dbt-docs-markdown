...
- `dbt clone` is only available with dbt version 1.6 and newer. Refer to our [upgrade guide](/docs/dbt-versions/upgrade-dbt-version-in-cloud) for help enabling newer versions in <Constant name="dbt" />.
...
Imagine you've created a [Slim CI job](/docs/deploy/continuous-integration) in <Constant name="dbt" /> and it is configured to: 
...
You'll have two commands for your <Constant name="dbt" /> CI check to execute:
